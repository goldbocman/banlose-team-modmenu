   ...
   // public native void Check();
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            HttpURLConnection urlConnection = (HttpURLConnection) new URL(UrlSite()).openConnection();
    ...
    protected void onPostExecute(String s) {
        ...
        if(s == null || s.isEmpty()){
            Toast.makeText(activity,(Html.fromHtml("<font face='Bold'> <font color='#f9f9f9'>Error!</font></font>")), Toast.LENGTH_LONG).show();
            return;
        }
        ...
        try {
            ...
            final String decData = Utils.profileDecrypt(ack.get("Data").toString(), ack.get("Hash").toString());
            final JSONObject data = new JSONObject(decData);
            if(data.get("Status").toString().equals("Success")) {
                //custom toast

                Toast.makeText(activity, (Html.fromHtml("<font face='Bold'> <font color='#f9f9f9'>Success!</font></font>")), Toast.LENGTH_SHORT).show();
                getActivity().startActivity(new Intent(activity, Class.forName(getActivity().sGameActivity)));
                getActivity().startService(new Intent(activity, MainActivity2.class));
